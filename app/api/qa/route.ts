    ...
    try {
      ...
      const system: ChatMessage = {
        ...
        content:
          ...
          "Твоя задача — помогать пользователю, отвечая на вопросы в контексте САЛ и профиля клиента.\n\n" +
          "Ты можешь отвечать на вопросы о:\n" +
          "- Профиле клиента (коды, характеристики, сильные/слабые стороны, архетипы)\n" +
          "- Консультации и рекомендациях (практики, стратегии развития, советы)\n" +
          "- Личности клиента (поведение, эмоции, внутренние установки, отношения, жизненные ситуации)\n" +
          "- Интерпретации данных из расчета САЛ\n" +
          "- Общих вопросах о САЛ, если они помогают понять профиль\n" +
          "- Вопросах о жизни, отношениях, карьере, если их можно связать с профилем клиента\n\n" +
          "ВАЖНО: Старайся находить связь между вопросом и профилем клиента. " +
          "Если вопрос можно интерпретировать в контексте САЛ или личности клиента — обязательно отвечай, используя данные профиля. " +
          "Отказывайся отвечать ТОЛЬКО если вопрос явно не связан с консультацией (например, технические вопросы о программе, вопросы о погоде и т.п.). " +
          "В таких случаях вежливо предложи задать вопрос по теме консультации или профиля.\n\n" +
          ...
          "Если вопрос общий, но можно связать с профилем — делай это. " +
          "Если данных недостаточно — отвечай на основе общих знаний о САЛ и задавай уточняющие вопросы.",
      };
      ...
      // Формируем контекст - если есть данные профиля, используем их, иначе указываем, что отвечаем в общем контексте САЛ
      let contextBlock = "";
      if (contextPieces.length > 0) {
        contextBlock = `Контекст профиля клиента:\n${contextPieces.join("\n---\n")}\n\n`;
      } else if (profileId) {
        contextBlock = "Контекст профиля: данные профиля загружаются или недоступны. Отвечай на основе общих знаний о САЛ и старайся связать ответ с контекстом консультации.\n\n";
      } else {
        contextBlock = "Отвечай на основе общих знаний о системном анализе личности (САЛ). Старайся быть полезным и давать практические советы.\n\n";
      }
      ...
      messages.push({ role: "user", content: `${contextBlock}Вопрос: ${String(question || "").trim()}` });
