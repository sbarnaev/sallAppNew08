...
2. **Проверьте, что переменные действительно переданы в контейнер**:
   ```bash
   docker-compose exec directus env | grep AUTH_TOKEN
   ```
   Должно вывести:
...
3. **Проверьте логи Directus при старте**:
   ```bash
   docker-compose logs directus | grep -i "auth\|token"
   ```

4. **Проверьте в Directus UI**:
   - Settings → Environment → Auth
   - Убедитесь, что значения `AUTH_TOKEN_TTL` и `AUTH_REFRESH_TOKEN_TTL` отображаются правильно

5. **Важно**: Токены, созданные ДО изменения настроек, будут иметь старый TTL (15 минут). 
   - Пользователям нужно **перелогиниться** после перезапуска контейнера
   - Новые токены будут иметь TTL 10 лет

6. **Если переменные не применяются**:
   - Убедитесь, что они находятся в секции `directus.environment`, а не в `database` или `cache`
   - Проверьте синтаксис YAML (отступы должны быть правильными)
   - Убедитесь, что нет опечаток в названиях переменных

7. **Альтернативный способ проверки**:
   Зайдите в Directus UI → Settings → Environment → Auth и проверьте значения напрямую.
   Если они не отображаются или показывают старые значения, значит переменные не применились.

8. **Если ничего не помогает**:
   - Попробуйте пересоздать контейнер: `docker-compose down -v` (осторожно: удалит volumes!)
   - Или добавьте переменные в `.env` файл и используйте их через `${AUTH_TOKEN_TTL}`
